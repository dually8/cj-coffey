...
async function generateImages(imagePath: string, outputDir: string) {
  try {
    ...
    console.error('Failed to generate images', (e as Error)?.message ?? 'Unknown error');
  }
}
// Check if correct arguments are provided
if (process.argv.length !== 4) {
  console.log('Usage: tsx generate-apple-icons.ts path/to/image.png path/to/outputDir');
  process.exit(1);
}
